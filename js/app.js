...
function genLink(key, comicName) {
    return 'javascript:del(\'' + key + '\',\'' + comicName + '\')';
}
...
// TODO: Get this working so it only displays subscriptions
        ...
        for(var j = 0; j < comicList.length; j++) {
            ...
            if (comicList[j].name == comic.series.name) {
                ...
                if (comic.series && comic.series.name != '') {
                    subscriptionData.removeLinks.push(genLink(comicList[j].key, comicList[j].name));
                }
...
$(document).ready(function () {
    ...
    $.when.apply($, promises).done(function () {
        ...
        if (res.code === 200) {
            ...
            $results.append(html);
            console.log("Marvel API finished");
        }
    });
